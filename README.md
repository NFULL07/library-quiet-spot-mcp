...
LibraryQuietSpot MCP는 Data4Library(도서관 정보나루) Open API의 실측 데이터를 가공해
...
| Tool | What it does |
|---|---|
| `find_best_visit_time` | 도서관별 이용 추이 데이터를 바탕으로 상대적으로 한산한 방문 시간대를 계산합니다. |
| `find_trending_books_and_library_match` | 지역 인기 도서 상위 5권을 지정 도서관의 소장 여부와 함께 보여줍니다. |
